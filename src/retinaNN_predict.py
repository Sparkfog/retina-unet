...
print("Fixing channels issue...")
# for layer in model.layers:
#     if 'conv' in layer.name:
#         w = layer.get_weights()
#         if len(w) > 0:
#             print(f"Before transpose: {w[0].shape}")  # Debugging
#             # if w[0].shape == (3,3,1,32):  # Detect wrong format
#             #     w[0] = np.transpose(w[0], (2, 3, 1, 0))  # Convert to (3,3,1,32)
#             # print(f"After transpose: {w[0].shape}")  # Debugging
#             layer.set_weights(w)


model.load_weights(path_experiment + name_experiment + '_' + best_last + '_weights_converted.h5')
